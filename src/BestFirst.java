...
public class BestFirst<T extends Comparable<T>> {
    ...
    private BiFunction<State, State, T> heuristic;

    public BestFirst(BiFunction<State, State, T> heuristicFoo) {
    ...
    public Three<State, LinkedList<State>, T> search(State ini, State end) {
        int nodesTreated = 0;
        Three<State, LinkedList<State>, T> initial = new Three<>(ini, new LinkedList<>(), this.heuristic.apply(ini, end));
        pending.add(initial);
        while (!pending.isEmpty()) {
            ...
            for(State succ : current.key().getSuccesors()) {
                if(!this.treated.containsKey(succ) && !this.pending.contains(succ)) {
                    ...
                    T costSucc = this.heuristic.apply(succ, end);
